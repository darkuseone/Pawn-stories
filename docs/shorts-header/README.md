...
**Выбор сделан: A и C, в конвейере жребием (`HEADER_STYLES` в
`pipeline/shorts.py`, id-стилей — `glass` и `soft`).** До этого в
конвейере был вариант, близкий к **D**, но без прозрачности и
скруглений — две плотных чёрных плашки; B (золотой кант) не взяли.
