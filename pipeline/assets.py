...
def main(job_path, stage="all"):
    ...
    # уехал в кэш. Падать надо там, где сломалось. Но только если картинки
    # вообще заказывались — пустой image_prompts (ролик целиком на
    # реальном футаже, например при нулевом балансе xAI) это осознанный
    # выбор спецификации, а не сломанная генерация, и роняться на нём
    # нельзя: 0 из 0 — это успех.
    made = len(list((work / "images").glob("img_*.jpg")))
    if not made and prompts:
        raise SystemExit(
