...
AXES = {
    ...
    # ── типографика ───────────────────────────────────────────────────
    # Карточка на числе. ВИД БОЛЬШЕ НЕ ЖРЕБИЙ: он один на канал (число —
    # акцентная линейка — подпись, см. editorial/textcard.py). Четыре
    # разных оформления на семи роликах читались как четыре разных
    # монтажёра, а не как разнообразие.
    #
    # Ось осталась ради style_override («none» вручную выключает карточки
    # конкретному ролику) и ради того, чтобы вектор стиля не терял поле.
    # Разнообразие ушло в СОСЕДНЮЮ ось: text_density растягивается в 1..3
    # карточки на ролик (textcard._want_count). У соседних роликов разное
    # ЧИСЛО карточек при одном виде — так и должно быть.
    "text_style":       dict(kind="pick", weight=0.4, options=["card"]),
