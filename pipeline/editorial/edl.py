...
------------
  ...
  timeline.md    человекочитаемый разбор: доли, замысел каждой, почему
                 кадр такой длины, где стоят выдохи и карточки
...
def write_timeline(shots, beats, style_card, pacing_report, findings,
    ...
    A(f"| Карточки на числах | `{style_card.get('text_style')}` | "
      f"число, акцентная линейка, подпись; редко и только на развязках |")
    ...
    # ── карточки на числах ──
    # Ключи берутся через .get: раньше здесь стояло m['text'] / m['style'] /
    # m['place'], и когда карточка сменила поля на value/unit/score, этот
    # блок начал ронять сборку KeyError-ом на САМОМ ПОСЛЕДНЕМ шаге —
    # после всего рендера, как когда-то youtube.py на отсутствующем
    # thumbnail.jpg. Монтажный лист — справка, и уронить из-за неё готовый
    # ролик нельзя ни при каком составе полей.
    if text_moments:
        A("## Карточки на числах")
        A("")
        A("Собственная графика конвейера: этих элементов нет ни в одном "
          "исходном материале. Ставятся редко и только на развязках, "
          "отбор по весу факта.")
        A("")
        A("| Тайм-код | Число | Подпись | Вес |")
        A("|---|---|---|---|")
        for m in text_moments:
            A(f"| {tc(m.get('t', 0), fps)} | {m.get('value', '')} | "
              f"{m.get('unit', '')} | {m.get('score', '')} |")
        A("")
