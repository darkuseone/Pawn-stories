...
# потому, что здесь живёт субтитр.
#
# КЕГЛЬ 116, А НЕ 58 — вдвое. На 58 подпись занимала 5% высоты кадра и на
# телефоне читалась хуже, чем интерфейс поверх неё; на просмотре готового
# ролика это первое, что видно. Побочный и полезный эффект: при вдвое
# большем кегле в две строки колонки влезает вдвое меньше слов, и событие
# субтитра стало короче — восемь-девять слов вместо семнадцати, то есть
# ровно то, что и должно висеть на экране за раз.
SUB_LONG_FS = 116

# ВЫСОТА СЧИТАЕТСЯ ОТ КЕГЛЯ, а не стоит числом. Блок выключен по центру
# вертикали (\an4), то есть две строки расходятся вверх и вниз от этой
# точки: на кегле 116 это ±139 px, и прежние 0.87 высоты увели бы нижнюю
# строку за край кадра (1078 из 1080). Считаем так, чтобы под блоком
# всегда оставалось SUB_LONG_BOTTOM пикселей до низа.
SUB_LONG_MAX_LINES = 2
SUB_LONG_BOTTOM = 78


def sub_long_y(h: int, fs: int = None, lines: int = None) -> int:
    """Центр блока субтитра по вертикали в пикселях."""
    fs = fs or SUB_LONG_FS
    lines = lines or SUB_LONG_MAX_LINES
    half = int(fs * 1.2 * lines / 2)
    return int(h - SUB_LONG_BOTTOM - half)
...
#
# Расширена с 0.66 до 0.82 вместе с удвоением кегля: на 116 в две трети
# кадра влезало бы по три-четыре слова в строку, и субтитр менялся бы
# чаще, чем его успевают прочитать.
SUB_LONG_COL = 0.82
...
#
# ОДИН ФАЙЛ ASS НА ВСЁ. Жжётся он внутри склейки групп (build.join): из
# одного графа выходят кадры с надписями и без. Всё, что рисуется буквами —
# название, субтитры с заливкой по словам и карточка-итог в конце, —
# libass рисует одним фильтром, и лишняя надпись не стоит ничего.
#
