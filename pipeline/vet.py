...
#
# "pinned" ЗДЕСЬ ПО ДРУГОЙ ПРИЧИНЕ, и она сильнее музейной. Закреплённый
# материал (assets.fetch_pinned) — это файл, который до пуша УЖЕ посмотрели
# глазами: превью скачали, ffmpeg вынул кадры, кадры оценили по смыслу под
# свой кусок сценария. Платить зрению за второй взгляд на то же самое
# незачем, а пересборок у ролика пять-десять.
TRUSTED_SOURCES = ("met", "pinned")
...
def triage(path: Path, im, bad, pale, src, query, current_queries, trusted):
    ...
    # ДОВЕРЕННЫЙ ИСТОЧНИК — СМОТРЕТЬ НЕЗАЧЕМ, и эта проверка стоит ПЕРЕД
    # проверкой запроса. Порядок был обратный, и на закреплённом материале
    # это молча стоило бы денег: у закреплённого файла в манифесте лежит
    # своё q (слова, под которые его подобрали в чате), а в
    # footage_queries / archive_queries остаётся только ЗАПАСНОЙ пул
    # словесных запросов. То есть q закреплённого файла заведомо «не из
    # текущей спецификации» — и проверка ниже отправляла бы к зрению
    # ровно тот материал, который человек уже посмотрел глазами, сколько
    # бы раз "pinned" ни стояло в trusted_sources.
    if src in trusted:
        return "keep", f"{src}: доверенный источник, второй взгляд не нужен"
