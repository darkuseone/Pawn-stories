...
def plan_shots(marks, st, assets, total, job_reject=None, job=None):
    ...
    # ГЕНЕРАЦИИ МОЖЕТ НЕ БЫТЬ ВОВСЕ, и это штатный режим, а не поломка.
    # Раньше пустая папка images роняла монтаж безусловно: генерация была
    # обязательной, потому что закрывала кадры, под которые «нет ни стока,
    # ни архива». Теперь материал подбирается и закрепляется ссылками в
    # чате, генерация опущена до 15% потолка и может быть нулевой — а на
    # ролике без ключа xAI её нет по определению. Падать надо только когда
    # закрыть кадр НЕЧЕМ ВООБЩЕ.
    if not images and not archive and not clips:
        raise SystemExit(
            f"в {assets} нет ни генерации, ни архива, ни футажа — "
            f"монтаж собрать нечем.\nПроверь закреплённый материал "
            f"(pinned_footage / pinned_archive) и запасные запросы: "
            f"скачалось ли хоть что-нибудь.")
    if not images and (archive or clips):
        log(f"  генерации нет — ролик идёт на реальном материале "
            f"(архив {len(archive)}, футаж {len(clips)})")
...
def main(job_path):
    ...
    # отсутствие названия, субтитров и карточки-итога. Шкала времени та же,
    # что у final.mp4, поэтому marks_final.json годится обоим.
    #
    # Без этого файла shorts.py вынужден резать из final.mp4 и выкручиваться:
    # либо замыливать полосу чужих субтитров, либо срезать низ кадра.
    if clean != silent:
        render.mux(clean, mixed, out / "clean.mp4")
        log(f"  clean.mp4 (без надписей, для шортсов): "
            f"{(out / 'clean.mp4').stat().st_size / 2**30:.2f} ГБ")
